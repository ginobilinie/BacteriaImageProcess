%function [x,f,funEvals,projects] = minConf_SPG(funObj,x,funProj,options,varargin)
...
[verbose,numDiff,optTol,progTol,maxIter,suffDec,interp,memory,useSpectral,curvilinear,feasibleInit,testOpt,bbType] = ...
    myProcessOptions(...
    options,'verbose',2,'numDiff',0,'optTol',1e-20,'progTol',1e-20,'maxIter',500,'suffDec',1e-4,...
...
if testOpt
    ...
    if max(abs(funProj(x-g)-x)) < optTol
        if verbose >= 1
        logPrint('First-Order Optimality Conditions Below optTol at Initial Point\n');
        end
        return;
    end
end
...
while funEvals <= maxIter
    ...
    %  if gtd > -progTol
    %      if verbose >= 1
    %          logPrint('Directional Derivative below progTol\n');
    %      end
    %      break;
    %  end
	...
	t = t/2;
        ...
        if t < temp*1e-3
            if verbose == 3
                logPrint('Interpolated value too small, Adjusting\n');
            end
            t = temp*1e-3;
        elseif t > temp*0.6
            if verbose == 3
                logPrint('Interpolated value too large, Adjusting\n');
            end
            t = temp*0.6;
        end
    ...
    % Check optimality
     if testOpt
         if optCond < optTol
             if verbose >= 1
             logPrint('First-Order Optimality Conditions Below optTol\n');
             end
             break;
         end
     end
     ...
     if max(abs(t*d)) < progTol
         if verbose >= 1
             logPrint('Step size below progTol\n');
         end
         break;
     end
     ...
     %     if verbose >= 1
     %         logPrint('Function value changing by less than progTol\n');
     %     end
     %     break;
     %end
    ...
    if funEvals*funEvalMultiplier > maxIter
        if verbose >= 1
            logPrint('Function Evaluations exceeds maxIter\n');
        end
        break;
    end
