...
if numDiff && method ~= TENSOR
    ...
    if method ~= NEWTON
        if debug
            if useComplex
                writelogfile(sprintf('Using complex differentials for gradient computation\n'));
            else
                writelogfile(sprintf('Using finite differences for gradient computation\n'));
            end
        end
        funObj = @autoGrad;
    else
    ...
    if method == NEWTON0 && useComplex == 1
        if debug
            writelogfile(sprintf('Turning off the use of complex differentials\n'));
        end
        useComplex = 0;
    end
...
if strcmp(DerivativeCheck,'on')
    if numDiff
        writelogfile(sprintf('Can not do derivative checking when numDiff is 1\n'));
    end
    % Check provided gradient/hessian function using numerical derivatives
    writelogfile(sprintf('Checking Gradient:\n'));
    [f2,g2] = autoGrad(x,useComplex,funObj,varargin{:});

    writelogfile(sprintf('Max difference between user and numerical gradient: %f\n',max(abs(g-g2))));
    if max(abs(g-g2)) > 1e-4
        writelogfile(sprintf('User NumDif:\n'));
        [g g2]
        diff = abs(g-g2)
        pause;
    end
    ...
    if method >= NEWTON
        writelogfile(sprintf('Check Hessian:\n'));
        [f2,g2,H2] = autoHess(x,useComplex,funObj,varargin{:});

        writelogfile(sprintf('Max difference between user and numerical hessian: %f\n',max(abs(H(:)-H2(:)))));
...
% Output Log
if verboseI
    writelogfile(sprintf('%10s %10s %15s %15s %15s\n','Iteration','FunEvals','Step Length','Function Val','Opt Cond'));
end
    ...
    switch method
        ...
        case CG % Non-Linear Conjugate Gradient
            ...
            if i == 1
                ...
                if g'*d > -tolX
                    if debug
                        writelogfile(sprintf('Restarting CG\n'));
                    end
                    beta = 0;
                    d = -g;
                end
...
%                 end
            ...
            if i == 1
                ...
                if i == 2
                    ...
                    if initialHessType == 0
                        ...
                        % Scaled Identity
                        if debug
                            writelogfile(sprintf('Scaling Initial Hessian Approximation\n'));
                        end
                        if qnUpdate <= 1
                ...
                if qnUpdate == 0 % Use BFGS updates
                    ...
                    if Damped
                        ...
                        if y'*s < eta*s'*Bs
                            if debug
                                writelogfile(sprintf('Damped Update\n'));
                            end
                        ...
                        if y'*s > 1e-10
                            ...
                            if debug
                                writelogfile(sprintf('Skipping Update\n'));
                            end
                        end
                    end
                elseif qnUpdate == 1 % Perform SR1 Update if it maintains positive-definiteness
                    ...
                    if abs(s'*ymBs) >= norm(s)*norm(ymBs)*1e-8 && (s-((R\(R'\y))))'*y > 1e-10
                        ...
                        if debug
                            writelogfile(sprintf('SR1 not positive-definite, doing BFGS Update\n'));
                        end
                        if Damped
                            eta = .02;
                            if y'*s < eta*s'*Bs
        ...
        case NEWTON0 % Hessian-Free Newton
            ...
            if debug
                writelogfile(sprintf('newtonCG stopped on iteration %d w/ residual %.5e\n',cgIter,cgRes));
            ...
            if useNegCurv
                if ~isempty(negCurv)
                    %if debug
                    writelogfile(sprintf('Using negative curvature direction\n'));
                    %end
            ...
            if cgSolve == 0
                if HessianModify == 0
                    ...
                    if posDef == 0
                        ...
                        if debug
                            writelogfile(sprintf('Adjusting Hessian\n'));
                        end
                ...
                elseif HessianModify == 4
                    ...
                    if posDef == 0
                        ...
                        if debug
                            writelogfile(sprintf('Taking Direction of Negative Curvature\n'));
                        end
                        [V,D] = eig(H);
                        u = V(:,1);
                        d = -sign(u'*g)*u;
                ...
                if debug
                    writelogfile(sprintf('CG stopped after %d iterations w/ residual %.5e\n',cgIter,cgRes));
                    %funEvals = funEvals + cgIter;
                end
            end
        ...
        case TENSOR % Tensor Method
            ...
            if any(abs(d) > 1e5) || all(abs(d) < 1e-5) || g'*d > -tolX
                if debug
                    writelogfile(sprintf('Using 2nd-Order Step\n'));
                end
                [V,D] = eig((H+H')/2);
                D = diag(D);
                D = max(abs(D),max(max(abs(D)),1)*1e-12);
                ...
                if debug
                    writelogfile(sprintf('Using 3rd-Order Step\n'));
                end
            end
    end

    if ~isLegal(d)
        writelogfile(sprintf('Step direction is illegal!\n'));
        pause;
        return
    end
    ...
    if verboseI
        writelogfile(sprintf('%10d %10d %15.5e %15.5e %15.5e\n',i,funEvals*funEvalMultiplier,t,f,sum(abs(g))));
    end
...
if verbose    
    ...
    if ~exist('msg','var') || isempty(msg)
        exitflag = 0;
        msg = '###### Warning, termination without valid error message';
    end
...
% Output Function
if ~isempty(outputFcn) && (maxIter > 0)
