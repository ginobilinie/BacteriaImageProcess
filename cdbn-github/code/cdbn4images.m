...
fname_prefix = sprintf('../results/cdbnmodel/crbm_new1h_%s_V1_w%d_b%02d_p%g_pl%g_plambda%g_sp%d_CD_eps%g_l2reg%g_bs%02d_%s',pars.dataname, pars.filtersize, pars.num_filters, pars.pbias, pars.pbias_lb, pars.pbias_lambda, pars.spacing, pars.learningRate, pars.l2reg, pars.batch_size, datestr(now, 30));
fname_save = sprintf('%s', fname_prefix);

%存weight的visualization结果和weight本身
fname_prefix1 = sprintf('../results/cdbnfigures/crbm_new1h_%s_V1_w%d_b%02d_p%g_pl%g_plambda%g_sp%d_CD_eps%g_l2reg%g_bs%02d_%s',pars.dataname, pars.filtersize, pars.num_filters, pars.pbias, pars.pbias_lb, pars.pbias_lambda, pars.spacing, pars.learningRate, pars.l2reg, pars.batch_size, datestr(now, 30));
...
%其实，这么做是不对的
for i=1:traindatasize%12557:-1 12558:+1
